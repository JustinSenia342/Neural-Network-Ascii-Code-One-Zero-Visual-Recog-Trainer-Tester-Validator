...
public class NeuralNetworkZeroOne
{
	/***********************************************************************************/
			...
			for (int j = 0; j < numInputAndOutput; j++)
			{
				...
				if (typeFlag == 'R')
				{
					if (j < numberInputs)
					{
						...
						pWrite.print(preProcArray[i][j] + " ");
						
						//used to keep data in 10x10 grid
						if ((j+1)%10 == 0)
						{pWrite.println("");}
					}
				}
		...
		for (int i = 0; i < numberRecords; i++)
		{
			...
			for (int j = 0; j < numberOutputs; j++)
			{
				if (typeFlag == 'V') //if validation file
				{
					...
					System.out.printf("(Actual Output = %.5f Predicted Output = %.5f)", temp1, temp2);
					System.out.printf("(Actual Output = %3d Predicted Output = %3d)", tempInt1, tempInt2);
					pWrite.printf("(Actual Output = %3d Predicted Output = %3d)", tempInt1, tempInt2);
				}
				else if (typeFlag == 'R') //if run/test file
				{
					...
					System.out.printf("%3d ", tempInt1);
					pWrite.printf("%3d ", tempInt1);
				}
			}
			System.out.println("");
			pWrite.println("");
